...
describe("conversations", () => {
  it("listConversations: GET with agent+scope+limit, envelope→array, snake→camel", async () => {
    const fetchMock = vi.fn(async () =>
      mk({ conversations: [{ id: "c1", agent_id: "marketing", title: "Q3", created_at: "t1", last_active_at: "t2", message_count: 4, scope: "corporate" }] }),
    );
    const client = new LibraOSClient({ baseUrl: "http://x", auth, fetch: fetchMock as unknown as typeof fetch });
    const out = await client.listConversations({ agentId: "marketing", scope: "corporate", limit: 20 });
    expect(out).toEqual([{ id: "c1", agentId: "marketing", title: "Q3", createdAt: "t1", lastActiveAt: "t2", messageCount: 4, projectId: null, scope: "corporate" }]);
    const [url] = fetchMock.mock.calls[0] as unknown as [string];
    expect(url).toContain("/v1/conversations");
    expect(url).toContain("agent=marketing");
    expect(url).toContain("scope=corporate");
    expect(url).toContain("limit=20");
  });

  it("getConversation: returns {conversation, messages} mapped", async () => {
    const fetchMock = vi.fn(async () =>
      mk({ id: "c1", agent_id: "marketing", title: null, created_at: "t1", last_active_at: "t2", message_count: 2, scope: "corporate",
           messages: [{ id: "m1", role: "user", content: "hi", timestamp: "ts1", seq: 1 }] }),
    );
    const client = new LibraOSClient({ baseUrl: "http://x", auth, fetch: fetchMock as unknown as typeof fetch });
    const out = await client.getConversation("c1");
    expect(out.conversation).toEqual({ id: "c1", agentId: "marketing", title: null, createdAt: "t1", lastActiveAt: "t2", messageCount: 2, projectId: null, scope: "corporate" });
  ...
  it("rejects on non-ok", async () => {
    const client = new LibraOSClient({ baseUrl: "http://x", auth, fetch: (async () => mk("e", 500)) as unknown as typeof fetch });
